...
\begin{itemize}
	...
	\item \textbf{Hito 5: Reto técnico y pivote estratégico.} Tras la implementación de las bases, se identificó que la creación de un motor de layout dinámico para los bloques de control de flujo complejos (ej. bucles anidados) era un desafío técnico mayor de lo previsto. En línea con la metodología ágil, se tomó la decisión estratégica de pivotar hacia un sistema híbrido reutilizando la implementación de Ublockly para la representación de bloques y la creación de un sistema para la codificación mediante texto. El objetivo era mantener una estructura visual pero introducir un DSL textual para los comandos, asegurando así la viabilidad del proyecto.

	\item \textbf{Hito 6: Desarrollo del sistema híbrido (MVP).} Implementación del CommandParser, el ProgramBuilder y el IDE\_UIManager. Adaptación del GameManager y RobotController para ejecutar la List<Instruction> generada por el parser. Diseño de la estructura de ChallengeData para los desafíos.
...
\begin{figure}[H]
	...
	\caption{Formato inicial de archivo XML para la configuración de los bloques y su jerarquía - Fase 1 del desarrollo.}
	\label{fig:xmlbloque}
\end{figure}
...
Se procedió a la redacción sistemática de cada uno de los capítulos y apéndices del documento, abordando las siguientes secciones:
	...
	\begin{itemize}
		\item \textbf{Arquitectura Inicial:} Se documentó la implementación del sistema de bloques visuales, la arquitectura MVC diseñada y los complejos sistemas de interacción como el BlockDragController y BlockConnectionController.
...
\item \textbf{Objetivo Principal:} Realizar un pivote estratégico en el enfoque del proyecto ante los desafíos técnicos encontrados con la generación de bloques complejos. A propuesta del tutor, el nuevo objetivo era desarrollar un prototipo funcional basado en un sistema de programación híbrido, que combinara una estructura de bloques visual para la organización del programa con un lenguaje de comandos textual (DSL) \cite{DSL}\cite{lenguaje_DSL} para las acciones del robot. Este sprint fue un punto de inflexión en el que se abordo la viabilidad de continuar o no el presente TFG. 
...
\begin{itemize}
	...
	\item \textbf{Pruebas del sistema híbrido:}
	\begin{itemize}
	...
	\end{itemize}
\end{itemize}
	\item \textbf{Diseño de una interfaz de programación híbrida (\texttt{IDE\_UIManager}):}
...
\begin{itemize}
		...
		\begin{itemize}
			\item Arrastrar y conectar los bloques de evento y movimiento (\texttt{motion\_movesteps}) que ya eran funcionales.
...
\item \textbf{Hito alcanzado:} Se completó con éxito el pivote estratégico del proyecto. Lo que en un principio abocaba a un fracaso en la creación del proyecto provoco un giro hacia un nuevo desarrollo. El resultado fue un prototipo MVP completamente funcional basado en un sistema híbrido y original, con un intérprete de comandos propio y un flujo de desafíos estructurado, regenerando la generación de bloques. Este hito no solo representa una solución técnica robusta a los problemas encontrados, sino también un enfoque pedagógico innovador que sirve de puente entre la programación visual y la textual.
...
\begin{itemize}
	...
	\item \textbf{Limitaciones del prototipo:} El prototipo actual implementa un conjunto limitado de bloques, comandos textuales y funcionalidades. La viabilidad de expandirlo a un sistema completo con todas las características de Scratch y comandos del DSL dependería de un esfuerzo de desarrollo continuado, pero las bases técnicas establecidas son sólidas.
