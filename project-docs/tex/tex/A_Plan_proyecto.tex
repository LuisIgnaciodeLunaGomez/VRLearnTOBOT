...
% \vspace{-3.0\baselineskip} %eliminación de espacios en blanco amplios
	...
	\begin{itemize}
	\item \textbf{GitHub:} Ha funcionado como el sistema de control de versiones (Git) y el repositorio centralizado para todo el código fuente. Una estrategia fundamental en su uso ha sido la \textbf{gestión mediante ramas} (branches) siendo un total de 17 las creadas. Cada nueva funcionalidad significativa o la corrección de errores complejos se abordó en una rama separada ejemplos de ramas utilizadas durante el desarrollo se pueden observar en la Figura \ref{fig:github_branches}).
...
\begin{itemize}
	...
	Configuración del entorno de desarrollo, análisis de la librería UBlockly para analizar la posibilidad de reutilizarla en el proyecto.
...
\item\textbf{Objetivo principal:} Producir una primera versión íntegra y de alta calidad de la memoria del Trabajo Fin de Grado, documentando todo el proceso, desde la investigación inicial y la implementación de la arquitectura de bloques.
...
\item \textbf{Objetivo Principal:} Realizar un pivote estratégico en el enfoque del proyecto ante los desafíos técnicos encontrados con la generación de bloques complejos. A propuesta del tutor, el nuevo objetivo era desarrollar un prototipo funcional basado en un sistema de programación híbrido, que combinara una estructura de bloques visual para la organización del programa con un lenguaje de comandos textual (DSL) para las acciones del robot.

\item \textbf{Contexto y motivación (El desafío de los bloques tipo C):}
El trabajo realizado en sprints anteriores había permitido establecer una arquitectura MVC robusta y un sistema funcional de arrastre y conexión para bloques simples. Sin embargo, al intentar implementar bloques de control de flujo complejos (los denominados bloques de tipo C, como \texttt{if/else} o bucles), se encontraron obstáculos técnicos que excedían las estimaciones iniciales. La replicación de estos bloques requería un motor de layout dinámico extremadamente complejo, capaz de redimensionar y reorganizar geométricamente los bloques contenedores en tiempo real, una tarea de una magnitud considerable

Este desafío representó un punto de inflexión crítico, no solo desde una perspectiva técnica, sino también metodológica. Puso de manifiesto una realidad fundamental en cualquier proyecto de ingeniería: la necesidad de tomar decisiones importantes para asegurar la entrega de un Producto Mínimo Viable (MVP) funcional cuando surgen complejidades imprevistas. El considerable esfuerzo y las horas dedicadas a la investigación y al desarrollo de la arquitectura inicial no podían quedar en un proyecto inacabado.
...
La nueva dirección, por tanto, no fue un abandono de los objetivos, sino una decisión consciente y pragmática. Se optó por un sistema híbrido que, manteniendo la claridad visual de un programa estructurado, introdujera la codificación textual manteniendo la programación por bloques adaptando el diseño de bloques al código diseñado en el repositorio de referencia. Este reenfoque permitió aprovechar todo el trabajo de base realizado (simulador 3D, arquitectura de control del robot) y centrar el esfuerzo restante en un nuevo núcleo técnico, el diseño de un DSL y su intérprete, que aportaría un valor pedagógico y técnico igualmente significativo.
\item \textbf{Tareas clave realizadas y resultados:}
\begin{itemize}
	\item \textbf{Diseño e implementación del intérprete de código textual:}
	\begin{itemize}
		\item \textbf{Definición del DSL:} Se diseñó la gramática para el lenguaje de comandos del robot, incluyendo mover, girar, y, crucialmente, las estructuras de control repetir N veces, repetir por\_siempre y su correspondiente cierre fin_repetir.
	...
	\begin{itemize}
		\item \textbf{Modo de programación textual (basado en DSL):} Se implementó una nueva vista para que los usuarios pudieran aprender a programar con código escrito y ver su ejecución inmediata en la simulación del robot. Esta vista incluye:
		\begin{itemize}
...
\item \textbf{Hito alcanzado:} Se completó con éxito el pivote estratégico del proyecto. El resultado es un prototipo MVP completamente funcional basado en un sistema híbrido y original, con un intérprete de comandos propio y un flujo de desafíos estructurado. Este hito no solo representa una solución técnica robusta a los problemas encontrados, sino también un enfoque pedagógico innovador que sirve de puente entre la programación visual y la textual.
...
\subsection{Costes de software y servicios}
\begin{itemize}
