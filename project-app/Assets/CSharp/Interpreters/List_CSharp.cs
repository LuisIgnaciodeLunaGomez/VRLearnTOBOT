...
using System.Text;
    ...
    {
        protected override DataStruct Execute(BlockModel block)
        {
    ...
    public class Lists_Create_With_Cmdtor : ValueCmdtor
    {
        protected override DataStruct Execute(BlockModel block)
        {
            ...
            if (mutator == null)
                throw new Exception("BlockModel \"lists_create_with\" must have a mutator \"lists_create_with_item_mutator\"");
    ...
    public static class ArrayListExtension
    {
        ...
        list = list.ConvertBoolean(); // Asegúrate que esta conversión maneje correctamente NaN si es necesario

        // Si la lista está vacía, ¿qué devolver? Quizás Number.NaN o lanzar excepción?
        // Asumiremos devolver 0 por ahora si está vacía, como el original.
        if (list.Count == 0)
        {
            return new DataStruct(Number.ZERO); // O Number.NaN según la lógica deseada
        }

        // --- CORRECCIÓN AQUÍ ---
        // Inicializa 'min' directamente con PositiveInfinity
        Number min = new Number(float.PositiveInfinity);
        // ----------------------

        for (int i = 0; i < list.Count; i++)
        {
            // Cuidado con NaN: Si un elemento es NaN, las comparaciones fallarán.
            // Deberías decidir cómo manejar NaN en Min/Max.
            Number tmp = ((DataStruct)list[i]).NumberValue;

            // La comparación 'min > tmp' podría necesitar ajuste si quieres ignorar NaN
            // o si NaN debe ser considerado el mínimo/máximo.
            // La lógica actual (con operadores sobrecargados) probablemente ignore NaN.
            if (tmp.IsNaN) continue; // Ejemplo: ignorar NaN

            if (min > tmp) // Usa el operador '>' sobrecargado
            {
                min = tmp; // Asigna la instancia completa 'tmp' a 'min'
            }
        }
        ...
        if (list.Count == 0)
        {
            return new DataStruct(Number.ZERO); // O Number.NaN
        }

        // --- CORRECCIÓN AQUÍ ---
        // Inicializa 'max' directamente con NegativeInfinity
        Number max = new Number(float.NegativeInfinity);
        // ----------------------
        ...
        for (int i = 0; i < list.Count; i++)
        {
            ...
            if (max < tmp) // Usa el operador '<' sobrecargado
            {
                max = tmp; // Asigna la instancia completa 'tmp' a 'max'
            }
        }
