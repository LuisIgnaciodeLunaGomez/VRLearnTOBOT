...
namespace UBlockly.UGUI
{
    public class BlockView : BaseView, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerClickHandler
    {
        ...
        public void BindModel(BlockModel block)
        {
        ...
        protected internal override void OnXYUpdated()
        {
            ...
            if (mBlock != null)
            {
               ...
               // Logger.Log($"<color=brown><b>[BlockView.OnXYUpdated]</b>:</color> Sincronizando modelo. mBlock.XY será {this.XY}", this.gameObject);
                mBlock.XY = this.XY;
            }
            base.OnXYUpdated();
        }
        ...
        {
            string origin = InToolbox ? "TOOLBOX" : "WORKSPACE";
           // Logger.Log($"<color=yellow><b>[BlockView.OnBeginDrag]</b> ({origin}):</color> Evento recibido para '{this.name}'", this.gameObject);


            if (!InToolbox)
            {
                BlocklyUI.WorkspaceController.BlockController.BeginDrag(this, eventData);
        ...
        public void OnDrag(PointerEventData eventData)
        {
           ...
           // Logger.Log($"<b>[BlockView.OnDrag]</b> ({origin})", this.gameObject);

            if (!InToolbox)
            {
                BlocklyUI.WorkspaceController.BlockController.Drag(this, eventData);
            }
        }
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            BlocklyUI.WorkspaceController.BlockController.EndDrag(this, eventData);
        ...
        /// </summary>
        protected void OnBlockUpdated(BlockModel.UpdateState updateState)
        {
            switch (updateState)
            {
                case BlockModel.UpdateState.Inputs:
                {
        ...
        private class MemorySafeBlockObserver : IObserver<int>
        {
            ...
            public void OnUpdated(object block, int updateStateMask)
            {
                ...
                {
                    ((BlockModel) block).RemoveObserver(this);
                }
                else
                {
                    foreach (BlockModel.UpdateState state in Enum.GetValues(typeof(BlockModel.UpdateState)))
                    {
