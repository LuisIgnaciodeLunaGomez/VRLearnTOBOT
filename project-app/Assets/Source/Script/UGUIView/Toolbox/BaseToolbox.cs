    ...
    public abstract class BaseToolbox : MonoBehaviour
    {
        ...
        protected BlockView NewBlockView(BlockModel block, Transform parent, int index = -1)
        {
            ...
            UIEventListener.Get(maskObj).onBeginDrag = data => PickBlockView(data, view);
               ...
               // Logger.Log($"<color=purple><b>[Toolbox.NewBlockView]:</b></color> Evento 'onBeginDrag' registrado para '{view.name}'. Delegará a PickBlockView.", view.gameObject);
        ...
        {
          //  Logger.Log($"<color=orange><b>[Toolbox] PickBlockView:</b></color> Iniciando el arrastre para el bloque de la toolbox '{blockView.name}'", blockView.gameObject);
            ...
            {
              //  Logger.LogError($"<color=red><b>[Toolbox] ERROR:</b></color> ¡CloneBlockView devolvió null! No se pudo crear el nuevo bloque.", blockView.gameObject);
                return; // Salimos si no se puede clonar.
            }

         //   Logger.Log($"<color=cyan><b>[Toolbox] PickBlockView:</b></color> Transfiriendo el evento de arrastre de '{data.pointerDrag.name}' a '{newBlockView.name}'");
          ...
          //  Logger.Log($"<color=cyan><b>[Toolbox] PickBlockView:</b></color> El objeto de arrastre ahora es '{data.pointerDrag.name}'");
        ...
        protected void BuildProcedureBlocks()
        {
            ...
            foreach (BlockModel block in mWorkspace.ProcedureDB.GetDefinitionBlocks())
            {
